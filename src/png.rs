...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, Box<dyn std::error::Error>> {
            ...
            .position(|chunk| chunk.chunk_type() == &chunk_type)
        {
            ...
            Err(format!("No chunk of type {} found", chunk_type))?
        }
    }
    ...
    // Consumes self writing everything to bytes
    ...
    pub fn read_to_png<R: BufRead>(r: &mut R) -> Result<Self, Box<dyn std::error::Error>> {
        let mut header: [u8; 8] = [0; 8];
        r.read_exact(&mut header)?;

        if header != Self::STANDARD_HEADER {
            Err("Invalid header for PNG file")?
        }

        let mut chunks: Vec<Chunk> = Vec::new();

        while let Some(chunk) = Chunk::read_to_chunk(r)? {
            chunks.push(chunk);
        }
...
mod tests {
    ...
    fn chunk_from_strings(
        ...
        let chunk_type = ChunkType::try_from(chunk_type.as_bytes())?;
    ...
    fn test_invalid_chunk() {
        ...
        let mut reader = BufReader::new(&chunk_bytes[..]);
        let png = Png::read_to_png(&mut reader);
    ...
    fn test_as_bytes() {
        let mut reader = BufReader::new(&PNG_FILE[..]);
        let png = Png::read_to_png(&mut reader).unwrap();
